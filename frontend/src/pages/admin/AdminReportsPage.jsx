...
export default function AdminReportsPage() {
 ...
 return <main className="dk-workspace"><span className="dk-eyebrow">A CLEARER PICTURE OF YOUR KITCHEN</span><h1>Sales analytics</h1><p>Paid, non-cancelled orders. Net food sales include discounts and exclude delivery fees.</p><button disabled={loading} onClick={()=>setRetry(n=>n+1)}>{loading?'Loading sales...':'Refresh sales'}</button>{error && <p role="alert">{error}</p>}
 <div className="inventory-summary">{[['Net food sales · 28 days','RM '+(summary?.analytics.net_food_sales || '0')],['Paid orders · 28 days',summary?.analytics.paid_orders || 0],['Average food order','RM '+(summary?.analytics.average_order_value || '0')],['Next 7 days · estimate',summary?.forecast.next_7_days_portions==null?'Insufficient history':summary.forecast.next_7_days_portions+' portions']].map(([label,value])=><article className="summary-card" key={label}><span>{label}</span><strong>{value}</strong></article>)}</div>
